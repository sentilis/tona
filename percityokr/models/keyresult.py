...
class KeyResult(BaseModel):
    ...
    end_date = DateField()
    status = CharField(max_length=10, default='draft') # off-track (<60) at-risk (>60 80<) on-track (>80)
...
def get_changes_metric(name):
    ...
    if find_changes_metric:
        changes_metric = find_metric_measure.group()
        if changes_metric == '-':
...
def keyresult_smart_name(name):
    """
        --add #12 My first KR
        --add Im write any /Q&A from +1.100 
        --
